...
FServerSideRewindResult ULagCompensationComponent::ConfirmHit(const FFramePackage& Package, ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation)
{
	...
	if (World)
	{
		...
		if (ConfirmHitResult.bBlockingHit) // we hit head, return early
		{
			//BlasterDebugBox(ConfirmHitResult, FPredictProjectilePathResult());
...
FServerSideRewindResult ULagCompensationComponent::ProjectileConfirmHit(const FFramePackage& Package, ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, float HitTime)
{
	...
	{
		//BlasterDebugBox(FHitResult(), PathResult);
...
FShotgunServerSideRewindResult ULagCompensationComponent::ShotgunConfirmHit(const TArray<FFramePackage>& FramePackages, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations)
{
	...
	{
		if (Frame.Character == nullptr) return FShotgunServerSideRewindResult(); // unclear if this is properly implemented
	}

	FShotgunServerSideRewindResult ShotgunResult; // stores all the pellet hits on the rewind meshes
	TArray<FFramePackage> CurrentFrames; // store the different rewind-hitbox-meshes from each potential character that was hit
	for (auto& Frame : FramePackages)
	{
		FFramePackage CurrentFrame;
	...
	// Enable collisions for all boxes, then disable for head boxes
	for (auto& Frame : FramePackages)
	{
...
FFramePackage ULagCompensationComponent::GetFrameToCheck(ABlasterCharacter* HitCharacter, float HitTime)
{
	...
	if (Older->GetValue().Time == HitTime) // really really unlikely
	{
...
void ULagCompensationComponent::BlasterDebugBox(FHitResult ConfirmHitResult, FPredictProjectilePathResult PathResult)
{
	UBoxComponent* Box = nullptr;

	if (ConfirmHitResult.Component.IsValid())
	{
		Box = Cast<UBoxComponent>(ConfirmHitResult.Component.Get());
	}
	else if (PathResult.HitResult.Component.IsValid())
	{
		Box = Cast<UBoxComponent>(PathResult.HitResult.Component.Get());
	}
	if (Box)
	{
