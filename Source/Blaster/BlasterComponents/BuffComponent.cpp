...
{
	bShielding = true;
	ShieldingRate = ShieldAmount / ReplenishTime;
...
void UBuffComponent::BuffJump(float BuffJumpyVelocity, float Bufftime)
{
	if (Character == nullptr) return;
	...
	if (Character->GetCharacterMovement())
	{
		Character->GetCharacterMovement()->JumpZVelocity = BuffJumpyVelocity;
	}

	MulticastJumpBuff(BuffJumpyVelocity);
}
